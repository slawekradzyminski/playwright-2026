...
Product creation rejects an empty description that the published schema permits. Contract-following callers cannot create that payload, but supplying a nonempty description gives a clear correction path. This is a request-contract incompatibility; the evidence does not establish that all product creation fails.
