...
Selecting Sign in from a valid registration form submits signup and creates a persistent account, verified by subsequent successful login. The navigation action reserves username/email without the user choosing Create account; navigating back does not undo that mutation. This compromises user control over account creation. The user explicitly confirmed this severity on 2026-09-09.
...
Severity raised to High at the user’s request on 2026-09-09 because navigation causes an unintended persistent account mutation. This is a triage update, not a new reproduction.
