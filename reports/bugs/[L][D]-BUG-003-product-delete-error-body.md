...
Deleting an already absent product returns the correct 404 status but omits the documented error body. The caller can determine the absent-resource outcome from that status without retrying a destructive action. A consumer parsing failure is possible but unobserved; no blocked deletion or state corruption is demonstrated.
