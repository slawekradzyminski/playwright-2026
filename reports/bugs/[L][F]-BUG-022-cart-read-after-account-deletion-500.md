...
A cart read with a deleted account's old token returns 500 instead of a controlled rejection. The account and its data were deleted successfully; the evidence shows no access to retained data, effect on active customers or failed deletion. Impact is misleading diagnostics for an already unusable identity, recoverable by ending that stale session; the intended rejection status remains unresolved.
