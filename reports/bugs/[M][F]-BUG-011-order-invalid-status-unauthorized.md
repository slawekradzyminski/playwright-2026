...
Malformed order status is reported as 401 even though the administrator token remains valid and a valid update succeeds. Callers cannot use the status to distinguish input correction from authentication recovery. Reauthentication loops are a possible consequence, not observed evidence; correcting the payload restores the supported operation.
