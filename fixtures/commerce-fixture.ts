  ...
  commerce: async ({ request, adminToken }, use) => {
    ...
    try {
      for (let index = 0; index < 2; index++) {
        ...
        const response = await login.login({ username: account.username, password: account.password });
        expect(response.status(), 'Authenticate commerce user').toBe(200);
        const body = await response.json();
        expect(body.roles).toEqual(['ROLE_CLIENT']);
        expect(body.token).toEqual(expect.stringMatching(/\S+/));
        users.push({ username: account.username, token: body.token });
      }
