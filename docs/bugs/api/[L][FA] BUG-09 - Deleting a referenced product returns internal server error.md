...
Deletion of an order-referenced product returns an opaque server error, but refusing that deletion may legitimately protect order history. The evidence establishes poor conflict handling, not that permitted product retirement is blocked. Deleting real order owners is not a valid production workaround; retirement policy still needs agreement.
...
Deleting a newly created product referenced by an order still returned 500 with Internal server error. Deleting the disposable order owner and then the product returned 204 for each. The dependent-data restriction may be legitimate; the opaque server error is the confirmed defect.
