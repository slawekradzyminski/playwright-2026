...
The documented failure status lacks models for field and duplicate errors, so consumers must map runtime bodies manually. The API gives readable feedback and no supported consumer failure or blocked valid registration was demonstrated. The consequence is additional integration work.
...
The unchanged live sign-up contract still omits documented error body models. A malformed request was reproduced, and the existing representative validation suite passes. The full original exploration matrix was not repeated.
