...
Malformed requests are rejected and valid login remains available. The demonstrated consequence is misleading authentication guidance during request repair; no lost session, recovery loop or accepted invalid request was shown. Correcting the request is a practical recovery path.
