...
Invalid status input receives misleading authentication guidance even though the same administrator credentials work for supported transitions. The recorded consequence is error classification and troubleshooting, without demonstrated session loss or unauthorized status mutation.
...
A valid administrator updating a newly created order to INVALID still received 401 with Unauthorized. Valid transitions remain usable. The observed issue is incorrect corrective guidance, not a security compromise.
