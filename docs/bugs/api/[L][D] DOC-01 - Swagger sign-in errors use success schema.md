...
Error responses advertise a login-success model, requiring manual error mapping. Failure statuses remain usable and the permissive schema may accept the body; neither a blocked valid login nor a generated-client outage was demonstrated. The established consequence is inaccurate integration guidance.
...
The live specification is unchanged and still assigns the successful login model to error responses. Fresh sign-in validation/authentication failures reproduce error maps rather than token responses. Misleading models are confirmed; a permissive schema may still accept extra properties, and no consumer outage was demonstrated.
