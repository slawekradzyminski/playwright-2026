...
Routine successful product operations return null and offset-free timestamp values incompatible with the declared types. Strict consumers need model overrides, and absolute timestamp interpretation requires timezone information outside the payload. This materially affects contract compatibility without proving a catalog outage.
...
Fresh product creation again returned imageUrl null and createdAt/updatedAt without an offset or UTC designator. The live ProductDto still declares a non-null string and date-time formats. The product was removed with 204. The recorded consequence is response-contract incompatibility, not a demonstrated catalog outage.
