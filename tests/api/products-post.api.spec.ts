...
for (const { label, overrides } of validProductCases) {
  test(`201 - create product with ${label}`, async ({ adminToken, productIds }) => {
    // given
...
for (const { label, field, value } of invalidCases) {
  test(`400 - reject ${label}`, async ({ adminToken, productIds }) => {
    // given
    ...
    if (body.id) productIds.add(body.id);
...
for (const { label, token, message } of unauthorizedCases) {
  test(`401 - reject ${label}`, async ({ adminToken, productIds }) => {
    // given
