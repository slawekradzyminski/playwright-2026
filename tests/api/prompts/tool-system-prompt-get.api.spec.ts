...
test('200 - customer receives the tool default', async ({ loggedInUser }) => {
  // given
  const firstToken = loggedInUser.token;
  // when
  const firstResponse = await client.getToolSystemPrompt(firstToken);

  // then
  const first = await expectToolSystemPrompt(firstResponse, 200);
  expect(first.toolSystemPrompt).toBeTruthy();
});
...
for (const scenario of ['anonymous', 'invalid bearer'] as const) {
  test(`401 - reject ${scenario} tool prompt lookup`, async () => {
    // given
