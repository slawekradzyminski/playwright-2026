...
test.describe('PUT /api/v1/users/tool-system-prompt', () => {
  let client: UpdateToolSystemPromptClient;
  let getClient: GetToolSystemPromptClient;

  test.beforeEach(async ({ request }) => {
    // given
    client = new UpdateToolSystemPromptClient(request);
    getClient = new GetToolSystemPromptClient(request);
  });
  ...
  test('should restore the default when the override is cleared - 200', async ({ account }) => {
    ...
    // then
    expect(response.status()).toBe(200);
    expect(await response.json()).toEqual({ toolSystemPrompt: '' });
  ...
  test('should reject an overlong prompt without changing the stored value - 400', async ({ account }) => {
    ...
    expect(await response.json()).toEqual({ toolSystemPrompt: 'Tool system prompt must be at most 5000 characters' });
    const stored = await getClient.get(token);
    expect(stored.status()).toBe(200);
    expect(await stored.json()).toEqual({ toolSystemPrompt: prompt });
  });
